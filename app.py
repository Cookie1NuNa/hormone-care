...
def display_dry_skin_habit():
    st.info("### 🌵 제2유형(악건성) CEO를 위한 '상시 업무 지침'")
    ...
    with col2:
        ...
        **4. 믹스(Mix) 기술 활용**
        * 피부가 유독 당기는 날엔?
        * 크림에 **'페이스 오일'**을 1~2방울 섞어서 바르면 비싼 영양 크림 안 부럽습니다.
        """)
...
def display_hormone_guide(day):
    ...
    st.markdown(f"#### 🧸 오늘의 보고서: **Day {day}**")
        ...
        # [유형파악] 건조함2 (접었다 폈다 할 수 있게)
    with st.expander("ℹ️ '건조함이 심한 제2유형'이 뭔가요? (내 피부 타입 확인)"):
        st.markdown("""
        **당신의 회사는 '알뜰 살림형 미니멀리스트' 기업입니다!** 🌵
        군더더기 없이 깔끔하게 운영되지만, 윤활유(수분/유분)가 늘 부족한 상태죠.
        
        * **👍 신의 축복 (장점):**
            * **무통 경영:** 남들이 배 잡고 뒹굴 때, 사장님은 평온합니다. (생리통 거의 없음) 😌
            * **청정 구역:** 피지 분비가 적어서 여드름이나 개기름 걱정이 거의 없습니다. ✨
        
        * **👎 경영 위기 (단점):**
            * **사막화 경보:** 얼굴뿐만 아니라 눈, 입, 온몸이 바짝바짝 마릅니다.
            * **조기 감가상각 (노화):** 기름칠(유분)이 부족해서, 관리 안 하면 남들보다 잔주름이 빨리 생겨요. 👵
        
        * **💡 CEO 경영 지침:**
            * **"물만 주지 말고, 기름을 줘라!"** (수분크림만으로는 부족해요! **페이스 오일** 필수 💧+🛢️)
            * **"외부 투자 유치!"** (콩, 두유, 석류 같은 **에스트로겐 식품**을 먹어서 채워주세요 🍒)
        """)
    ...
    if 1 <= day <= 5:
        ...
        if day <= 2:
            ...
            * **미백 시동:** 비타민C 앰플 꺼내두세요!
            """)
            st.write("---")
            st.write(f"🌙 저녁: {make_routine(['📍효소파우더', '토너', '💧디오디너리 히알루론산', '프리메이/세타필'])}")
    ...
    # 2. 난포기 (Day 6 ~ 13)
        ...
        **피부와 컨디션이 최고로 좋은 시기**입니다. 회복력이 빨라서 뭘 해도 잘 돼요.
        ...
        if day == 6:
            ...
            st.warning(f"🌙 루틴: {make_routine(['토너', '🍋코스알엑스 비타민C', '🧖‍♀️마스크팩(진정)', '🛡️마데카/시카밤'])}")
            
        elif day == 7:
            st.markdown("#### 🚀 Day 7: 디오디너리 알부틴 + 기기 (톤 정리)")
            st.write("**목표:** 뷰티 디바이스가 있다면 오늘이 기회입니다. 알부틴을 피부 깊숙이 밀어 넣으세요.")
            st.warning(f"🌙 루틴: {make_routine(['토너', '💧디오디너리 히알루론산', '💡디오디너리 알부틴(+기기)', '프리메이/세타필'])}")
            ...
            st.warning(f"🌙 루틴: {make_routine(['토너', '💧디오디너리 히알루론산', '🧴크림+나이아신 믹스', '한겹 더 덮기'])}")
            ...
            * **크림 조절:** 13일쯤 되면 피지가 슬슬 올라오니 꾸덕한 크림 양을 조금 줄이세요.
            """)
            st.warning(f"🌙 루틴: {make_routine(['토너', '💧디오디너리 히알루론산', '🧖‍♀️수분 마스크팩', '마데카/세타필'])}")
    ...
    # 3. 배란기 & 체제 전환 (Day 14 ~ 16)
        ...
        if day == 14:
            ...
            **💬 현실 해석:**
            ...
            st.success("✅ **추천:** 오일 클렌징으로 피지 녹이기, 가벼운 수분 크림")
            st.write("---")
            st.warning(f"🌙 루틴: {make_routine(['오일클렌징', '토너', '🧊모델링/머드팩', '프리메이 수분크림'])}")
            ...
            **💬 현실 해석:**
            ...
            st.write("---")
            st.warning(f"🌙 루틴: {make_routine(['🧊차가운 토너', '💧디오디너리 히알루론산', '알로에/프리메이', '🛡️시카밤(얇게)'])}")
